...
_gulp.task("min:js", function () {
	return _gulp.src([paths.js, "!" + paths.minJs], { base: "." }).pipe(_concat(paths.concatJsDest)).pipe(_uglify()).pipe(_gulp.dest("."));
});

_gulp.task("min:css", function () {
	return _gulp.src([paths.css, "!" + paths.minCss]).pipe(_concat(paths.concatCssDest)).pipe(_cssmin()).pipe(_gulp.dest("."));
});

_gulp.task("sass", function () {
	return _gulp.src([paths.Scss]).pipe(_sass()).pipe(_gulp.dest('./css'));
});
